    ...
    <div class="panel panel-primary">
        ...
        <c:forEach var="waitorder" items="${waiting}">
            <div class="panel-body">
                <div class="col-lg-6">
                    <f:message key="car.mark"/>: ${waitorder.car.mark}<br/>
                    <f:message key="car.model"/>: ${waitorder.car.model}<br/>
                    <f:message key="car.carClass"/>: ${waitorder.car.carClass.name}<br/>
                    <f:message key="car.costPerDay"/>: ${waitorder.car.price}<br/>
                </div>
                <div class="col-lg-6">
                    <f:message key="user"/> <f:message key="order.passport"/>: ${waitorder.passport}<br/>
                    <f:message key="order.startDate"/>: ${waitorder.startDate}<br/>
                    <f:message key="order.endDate"/>: ${waitorder.endDate}<br/>
                    <f:message key="order.driver"/>: ${waitorder.driver}<br/>
                    <f:message key="order"/> <f:message key="global.status"/>: ${waitorder.status.name}<br/>
                </div>
            </div>
        </c:forEach>
        <div class="row">
            <div class="col-lg-2">
                <form method="post" action="${pageContext.request.contextPath}/makeCheck">
                    <button type="submit" class="btn btn-warning"><f:message key="makeCheck.button"/></button>
                </form>
            </div>
            <div class="col-lg-2">
                <form action="${pageContext.request.contextPath}/deleteTempOrders" method="post">
                    <button type="submit" class="btn btn-danger"><f:message key="admin.button.delete"/></button>
                </form>
            </div>
        </div>
    </div>
...
<c:if test="${not empty checks}">
    ...
    <c:forEach var="check" items="${checks}">
        <div class="panel panel-success">
            <div class="panel-body">
                <div class="col-lg-6">
                    <p class="h4"><f:message key="order"/></p>
                ...
                <div class="col-lg-6">
                    <p class="h4"><f:message key="check"/>: ${check.id}</p><br/>
                    <f:message key="order.currentDate"/>: ${check.date}<br/>
                    <f:message key="check.description"/>: ${check.description}<br/>
                    <f:message key="global.status"/>: ${check.status.name}<br/>

                    <div class="h3">
                        <f:message key="check.totalCost"/>: ${check.price}
                    </div>
                </div>
                <c:choose>
                    ...
                    <c:when test="${check.status.name=='canceled'}">
                        <div class="col-lg-offset-5 col-lg-2">
                            <p class="h2" style="color: red"><f:message key="user.info.orderCanceled"/></p>
                        </div>
                        ...
                        <c:choose>
                            <c:when test="${status=='rent'}">
                                <form action="${pageContext.request.contextPath}/returnCar" method="post">
                                    <input name="id" value="${check.id}" >
                                    <button type="submit" class="btn btn-warning"><f:message
                                            key="user.button.returnCar"/></button>
                                </form>
                            </c:when>
                            <c:when test="${status=='returned'}">
                                <p class="h2 bg-info"><f:message key="user.info.carBroken"/></p>
                            </c:when>
                        </c:choose>
                    </c:when>
                </c:choose>
